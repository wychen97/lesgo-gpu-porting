...
!
! Navigation map:
!   - type declarations: turbine array, turbine model, blade, nacelle, airfoils
!   - case input: read_input_conf
!   - turbine model input: read_turbine_model_variables
!   - startup reporting: atm_print_initialize
!   - airfoil tables: read_airfoil
!   - parser helpers: readline and eat_whitespace
!
! Keep input defaults and new fields synchronized with actuator_turbine_model.f90
...
! Optional heavy ATM diagnostic cadence. Negative preserves legacy behavior:
! use outputInterval for all blade/airfoil diagnostic files. Zero disables
! heavy diagnostics while keeping scalar power/thrust output active.
integer :: diagnosticOutputInterval = -1
...
type turbineArray_t
    ...
    real(rprec), allocatable, dimension(:,:,:) :: flap_disp
    real(rprec), allocatable, dimension(:,:,:) :: flap_vel
    real(rprec), allocatable, dimension(:,:,:) :: flap_acc
    real(rprec), allocatable, dimension(:,:,:) :: theta_disp
    real(rprec), allocatable, dimension(:,:,:) :: theta_vel
    real(rprec), allocatable, dimension(:,:,:) :: theta_acc
    real(rprec), allocatable, dimension(:,:,:) :: edge_disp
    real(rprec), allocatable, dimension(:,:,:) :: edge_vel
    real(rprec), allocatable, dimension(:,:,:) :: edge_acc
...
type turbineModel_t
    ...
    real(rprec), dimension(100) :: EI, EdgStff, GJStff, rho
...
!            write(*,*)  'updateInterval is: ', updateInterval
            cycle
        else if( buff(1:24) == 'diagnosticOutputInterval' ) then
            read(buff(25:), *) diagnosticOutputInterval
            cycle
        endif
...
if (diagnosticOutputInterval < 0) diagnosticOutputInterval = outputInterval
...
!            write(*,*) 'GenIner is: ', turbineModel(i) % GenIner
        ...
        if ( buff(1:9) .eq. 'BladeData' ) then ! Start reading blade data block
                ...
                do while (array_exit_pos .eq. 0)
                    ...
                    turbineModel(i) % EI(NumSec) = 0._rprec
                    turbineModel(i) % EdgStff(NumSec) = 0._rprec
                    turbineModel(i) % GJStff(NumSec) = 0._rprec
                    turbineModel(i) % rho(NumSec) = 0._rprec

                    ! Extended line-model files append flap/edge/torsion
                    ! stiffness and line mass density after the airfoil id.
                    read(buff,*,iostat=blade_ios)                              &
                    turbineModel(i) % radius(NumSec),                          &
                    turbineModel(i) % chord(NumSec),                           &
                    turbineModel(i) % twist(NumSec),                           &
                    turbineModel(i) % sectionType(NumSec),                     &
                    turbineModel(i) % EI(NumSec),                              &
                    turbineModel(i) % EdgStff(NumSec),                         &
                    turbineModel(i) % GJStff(NumSec),                          &
                    turbineModel(i) % rho(NumSec)
                    if (blade_ios /= 0) then
...
numBl=turbineModel(j) % numBl
    ...
    allocate(turbineArray(i) % bladePoints_rigid(numBl,                        &
             numAnnulusSections, numBladePoints,3) )
    ...
    allocate(turbineArray(i) % Cd(numBl,                                       &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % Cm(numBl,                                       &
             numAnnulusSections, numBladePoints) )
    ...
    allocate(turbineArray(i) % tangentialForce(numBl,                          &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % pitchingMoment(numBl,                           &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % elastic_twist(numBl,                            &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % flap_disp(numBl,                                &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % flap_vel(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % flap_acc(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % theta_disp(numBl,                               &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % theta_vel(numBl,                                &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % theta_acc(numBl,                                &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % edge_disp(numBl,                                &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % edge_vel(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % edge_acc(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % edge_theta_disp(numBl,                          &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % edge_theta_vel(numBl,                           &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % edge_theta_acc(numBl,                           &
             numAnnulusSections, numBladePoints) )
    ...
    allocate(turbineArray(i) % chord(numBl,                                    &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % EI_blade(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % EI_edge_blade(numBl,                            &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % GJ_blade(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % rho_blade(numBl,                                &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % twistAng(numBl,                                 &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % sectionType(numBl,                              &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % sectionTypeBlendLo(numBl,                       &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % sectionTypeBlendHi(numBl,                       &
             numAnnulusSections, numBladePoints) )
    allocate(turbineArray(i) % sectionTypeBlendW(numBl,                        &
             numAnnulusSections, numBladePoints) )
    ...
    ! Initialize transient aerodynamic/structural state.  Structure-on runs call
    ! atm_solve_structure() before the first blade-force update, so the previous
    ! force/airfoil state must be deterministic immediately after allocation.
    ...
    turbineArray(i) % dalpha(:,:,:) = 0._rprec
    ...
    turbineArray(i) % epsilon_opt(:,:,:) = 0._rprec
    turbineArray(i) % uy_LES(:,:,:) = 0._rprec
    turbineArray(i) % uy_opt(:,:,:) = 0._rprec
    turbineArray(i) % Uinf_vec(:,:,:,:) = 0._rprec
    turbineArray(i) % uy_LES_vec(:,:,:,:) = 0._rprec
    turbineArray(i) % ux_LES_vec(:,:,:,:) = 0._rprec
    turbineArray(i) % uy_opt_vec(:,:,:,:) = 0._rprec
    turbineArray(i) % du(:,:,:,:) = 0._rprec
    turbineArray(i) % G(:,:,:) = 0._rprec
    turbineArray(i) % dG(:,:,:) = 0._rprec
    turbineArray(i) % Cl_b(:,:,:) = 0._rprec
    turbineArray(i) % dCldalpha(:,:,:) = 0._rprec
    turbineArray(i) % bladeVectorDummy(:,:,:,:) = 0._rprec
    turbineArray(i) % bladeScalarDummy(:,:,:) = 0._rprec
    turbineArray(i) % bladePoints_rigid(:,:,:,:) = 0._rprec
    turbineArray(i) % elastic_twist(:,:,:) = 0._rprec
    turbineArray(i) % flap_disp(:,:,:) = 0._rprec
    turbineArray(i) % flap_vel(:,:,:) = 0._rprec
    turbineArray(i) % flap_acc(:,:,:) = 0._rprec
    turbineArray(i) % theta_disp(:,:,:) = 0._rprec
    turbineArray(i) % theta_vel(:,:,:) = 0._rprec
    turbineArray(i) % theta_acc(:,:,:) = 0._rprec
    turbineArray(i) % edge_disp(:,:,:) = 0._rprec
    turbineArray(i) % edge_vel(:,:,:) = 0._rprec
    turbineArray(i) % edge_acc(:,:,:) = 0._rprec
    turbineArray(i) % edge_theta_disp(:,:,:) = 0._rprec
    turbineArray(i) % edge_theta_vel(:,:,:) = 0._rprec
    turbineArray(i) % edge_theta_acc(:,:,:) = 0._rprec
