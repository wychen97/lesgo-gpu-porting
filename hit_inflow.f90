...
if (hit_cuda_enabled()) call hit_gpu_setup(hit % Nx, hit % Ny, hit % Nz,       &
    hit % x, hit % y, hit % z, hit % u, hit % v, hit % w)
...
if (hit_cuda_enabled()) call hit_fringe_setup_gpu(hit_fringe%nx,             &
    hit_fringe%iwrap, hit_fringe%alpha, hit_fringe%beta)
...
if (hit_cuda_enabled()) then
    call hit_compute_plane_gpu(hit % xloc, hit % TI_out, inflow_velocity)
else
    do j = 1, Ny
        do k = 1, Nz
...
if (hit_cuda_enabled()) then
    call hit_apply_fringe_gpu(hit_fringe%nx, hit_fringe%iwrap,                &
        hit_fringe%alpha, hit_fringe%beta, u, v, w)
else
