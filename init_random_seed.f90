...
!
  ...
  ! Non-Intel builds need an explicit getpid declaration.
#ifndef __INTEL_COMPILER
  integer, external :: getpid
#endif

  integer, allocatable :: seed(:)
  integer :: i, n, un, istat, dt(8), pid, env_stat, read_stat
  integer(int64) :: t
...
call get_environment_variable('LESGO_RANDOM_SEED', seed_env, status=env_stat)
if (env_stat == 0) then
   read(seed_env, *, iostat=read_stat) t
   if (read_stat == 0) then
      do i = 1, n
         seed(i) = lcg(t)
      end do
