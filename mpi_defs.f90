...
integer :: ip, coords(1)
integer :: localComm
logical :: use_cps_comm

! Set the local communicator
#ifdef PPCPS
    ! A binary may be compiled with CPS support but run a non-CPS inflow case.
    ! The full input parser has not run yet, so do a narrow early scan for
    ! inflow_type before deciding whether to split MPI_COMM_WORLD into the
    ! red/blue CPS communicators.
    use_cps_comm = cps_inflow_selected_from_input()
    if (use_cps_comm) then
...
#ifdef PPCPS
!*******************************************************************************
logical function cps_inflow_selected_from_input()
!*******************************************************************************
! Read only inflow_type from lesgo.conf before the full input parser runs.  This
! keeps USE_CPS=ON binaries usable for ordinary non-CPS inflow cases.
!*******************************************************************************
use param, only : path, CHAR_BUFF_LENGTH
use string_util, only : uppercase
implicit none

integer :: lun, ios, equal_pos, comment_pos
integer :: early_inflow_type
logical :: exst
character(CHAR_BUFF_LENGTH) :: buff
character(CHAR_BUFF_LENGTH) :: key
character(:), allocatable :: input_conf

cps_inflow_selected_from_input = .false.
early_inflow_type = 0

if (allocated(path)) then
    allocate(input_conf, source=path // 'lesgo.conf')
else
    allocate(input_conf, source='./lesgo.conf')
end if

inquire(file=input_conf, exist=exst)
if (.not. exst) return

open(newunit=lun, file=input_conf, action='read', status='old', iostat=ios)
if (ios /= 0) return
...
do
    ...
    comment_pos = index(buff, '!')
    if (comment_pos == 1) cycle
    if (comment_pos > 1) buff = buff(:comment_pos-1)

    equal_pos = index(buff, '=')
    if (equal_pos <= 1) cycle

    key = uppercase(adjustl(buff(:equal_pos-1)))
    if (trim(key) == 'INFLOW_TYPE') then
        read(buff(equal_pos+1:), *, iostat=ios) early_inflow_type
        if (ios == 0) cps_inflow_selected_from_input = (early_inflow_type == 4)
        exit
    end if
end do
...
end function cps_inflow_selected_from_input
...
if (world_np < 2 .or. modulo(world_np, 2) /= 0) then
    write(*,*) 'CPS inflow requires an even number of MPI ranks >= 2; world_np=', &
        world_np
...
integer :: nb_reqs(4)
integer :: nb_status(MPI_STATUS_SIZE, 4)
integer :: nb_env_status
character(len=32) :: nb_env
logical, save :: nb_checked = .false.
logical, save :: nb_enabled = .false.
...
call mpi_dbg_sendrecv_r (var(1,1,1), mpi_datasize, MPI_RPREC, down, 1,        &
    var(1,1,ubz), mpi_datasize, MPI_RPREC, up, 1, comm, status, ierr,         &
    'mpi_sync_down')
...
call mpi_dbg_sendrecv_r (var(1,1,ubz-1), mpi_datasize, MPI_RPREC, up, 2,      &
    var(1,1,0), mpi_datasize, MPI_RPREC, down, 2, comm, status, ierr,         &
    'mpi_sync_up')

end subroutine sync_up

!*******************************************************************************
subroutine sync_downup_nb()
!*******************************************************************************
implicit none

nb_reqs = MPI_REQUEST_NULL
call mpi_irecv(var(1,1,ubz), mpi_datasize, MPI_RPREC, up, 1, comm,             &
    nb_reqs(1), ierr)
call mpi_irecv(var(1,1,0), mpi_datasize, MPI_RPREC, down, 2, comm,             &
    nb_reqs(2), ierr)
call mpi_isend(var(1,1,1), mpi_datasize, MPI_RPREC, down, 1, comm,             &
    nb_reqs(3), ierr)
call mpi_isend(var(1,1,ubz-1), mpi_datasize, MPI_RPREC, up, 2, comm,           &
    nb_reqs(4), ierr)
call mpi_waitall(4, nb_reqs, nb_status, ierr)

end subroutine sync_downup_nb
