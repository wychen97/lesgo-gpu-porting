...
! FFTW plan creation needs representative work arrays with the final transform
! shapes.  They are deallocated immediately after the plans are created.
allocate( data(ld, ny) )
...
#ifdef PPLES_GPU
! kx, ky, k2 are read-only after this point. For the explicit-residency
! (mem:separate) build, push them to the device once so the spectral kernels
! (e.g. filt_da_gpu) find them under their present() clauses.
!$acc enter data copyin(kx, ky, k2)
#endif
