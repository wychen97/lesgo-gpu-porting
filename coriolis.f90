...
#ifdef PPMPI
    coriolis_reduce_send(1) = ubar
    coriolis_reduce_send(2) = vbar
    call MPI_AllReduce(coriolis_reduce_send, coriolis_reduce_recv, 2,          &
        MPI_RPREC, MPI_SUM, comm, ierr)
    ubar = coriolis_reduce_recv(1)
    vbar = coriolis_reduce_recv(2)
#endif
