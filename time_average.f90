...
!*******************************************************************************
! Navigation map:
!   - tavg_t: owns accumulated means, second moments, stresses, pressure,
!     forcing, Cs, and vorticity statistics
!   - init: allocate or read existing averaging state
!   - compute: accumulate one timestep into the averaging state
!   - finalize: release averaging buffers
!   - checkpoint: write restartable averaging state
!   - CGNS writers: optional PPCGNS output helpers
!
! Time-averaged output is part of the data contract for post-processing.
! Document format or averaging-window changes before changing write behavior.
...
! added.
! The parent descriptor must be present because the accumulation kernel
! references allocatable components through this%...
!$acc enter data copyin(this)
!$acc enter data copyin(this%u, this%v, this%w, this%u_w, this%v_w, this%w_uv)
!$acc enter data copyin(this%u2, this%v2, this%w2, this%uv, this%uw, this%vw)
!$acc enter data copyin(this%txx, this%tyy, this%tzz, this%txy, this%txz, this%tyz)
!$acc enter data copyin(this%p, this%fx, this%fy, this%fz, this%cs_opt2)
!$acc enter data copyin(this%vortx, this%vorty, this%vortz)
#endif
...
real(rprec) :: dt_loc
real(rprec) :: wuv_l, uw_l, vw_l, pres_l
...
if (nproc >= 1) then
    ...
    !$acc& private(wuv_l, uw_l, vw_l, pres_l, vortx_l, vorty_l, vortz_l,      &
    !$acc&         tmp_cur, tmp_prev, fzuv_l)
    do k = lbz, nz
    do j = 1, ny
    do i = 1, nx
        if (k < nz) then
            wuv_l = 0.5_rprec * (w(i,j,k+1) + w(i,j,k))
#if defined(PPTURBINES) || defined(PPATM) || defined(PPLVLSET)
            fzuv_l = 0.5_rprec * (fza(i,j,k+1) + fza(i,j,k))
#else
            fzuv_l = 0._rprec
#endif
        else
            wuv_l = 0.5_rprec * (w(i,j,k) + w(i,j,k-1))
#if defined(PPTURBINES) || defined(PPATM) || defined(PPLVLSET)
            fzuv_l = 0.5_rprec * (fza(i,j,k) + fza(i,j,k-1))
#else
            fzuv_l = 0._rprec
#endif
        end if
        ...
        if (k > lbz) then
            uw_l = 0.5_rprec * (u(i,j,k-1) + u(i,j,k))
            vw_l = 0.5_rprec * (v(i,j,k-1) + v(i,j,k))
            tmp_cur = dvdx(i,j,k) - dudy(i,j,k)
            tmp_prev = dvdx(i,j,k-1) - dudy(i,j,k-1)
            vortz_l = 0.5_rprec * (tmp_prev + tmp_cur)
        else
            uw_l = 0._rprec
            vw_l = 0._rprec
            vortz_l = 0._rprec
        ...
        if (coord == 0 .and. lbc_mom > 0 .and. k == 1) then
            uw_l = 0._rprec
            vw_l = 0._rprec
        end if
        if (coord == 0 .and. k == 1) vortz_l = 0._rprec
        if (coord == nproc-1 .and. ubc_mom > 0 .and. k == nz) then
            uw_l = 0._rprec
            vw_l = 0._rprec
        end if

        pres_l = p(i,j,k) - 0.5_rprec *                                      &
            (u(i,j,k)*u(i,j,k) + wuv_l*wuv_l + v(i,j,k)*v(i,j,k))
        vortx_l = dwdy(i,j,k) - dvdz(i,j,k)
        vorty_l = dudz(i,j,k) - dwdx(i,j,k)

        this%u(i,j,k) = this%u(i,j,k) + u(i,j,k) * dt_loc
        this%v(i,j,k) = this%v(i,j,k) + v(i,j,k) * dt_loc
        this%w(i,j,k) = this%w(i,j,k) + w(i,j,k) * dt_loc
        this%w_uv(i,j,k) = this%w_uv(i,j,k) + wuv_l * dt_loc
        this%u_w(i,j,k) = this%u_w(i,j,k) + uw_l * dt_loc
        this%v_w(i,j,k) = this%v_w(i,j,k) + vw_l * dt_loc

        this%u2(i,j,k) = this%u2(i,j,k) + u(i,j,k)*u(i,j,k) * dt_loc
        this%v2(i,j,k) = this%v2(i,j,k) + v(i,j,k)*v(i,j,k) * dt_loc
        this%w2(i,j,k) = this%w2(i,j,k) + w(i,j,k)*w(i,j,k) * dt_loc
        this%uv(i,j,k) = this%uv(i,j,k) + u(i,j,k)*v(i,j,k) * dt_loc
        this%uw(i,j,k) = this%uw(i,j,k) + uw_l*w(i,j,k) * dt_loc
        this%vw(i,j,k) = this%vw(i,j,k) + vw_l*w(i,j,k) * dt_loc

        this%txx(i,j,k) = this%txx(i,j,k) + txx(i,j,k) * dt_loc
        this%tyy(i,j,k) = this%tyy(i,j,k) + tyy(i,j,k) * dt_loc
        this%tzz(i,j,k) = this%tzz(i,j,k) + tzz(i,j,k) * dt_loc
        this%txy(i,j,k) = this%txy(i,j,k) + txy(i,j,k) * dt_loc
        this%txz(i,j,k) = this%txz(i,j,k) + txz(i,j,k) * dt_loc
        this%tyz(i,j,k) = this%tyz(i,j,k) + tyz(i,j,k) * dt_loc

        this%p(i,j,k) = this%p(i,j,k) + pres_l * dt_loc
#if defined(PPTURBINES) || defined(PPATM) || defined(PPLVLSET)
        this%fx(i,j,k) = this%fx(i,j,k) + fxa(i,j,k) * dt_loc
        this%fy(i,j,k) = this%fy(i,j,k) + fya(i,j,k) * dt_loc
        this%fz(i,j,k) = this%fz(i,j,k) + fzuv_l * dt_loc
#endif
        this%cs_opt2(i,j,k) = this%cs_opt2(i,j,k) + Cs_opt2(i,j,k) * dt_loc

        this%vortx(i,j,k) = this%vortx(i,j,k) + vortx_l * dt_loc
        this%vorty(i,j,k) = this%vorty(i,j,k) + vorty_l * dt_loc
        this%vortz(i,j,k) = this%vortz(i,j,k) + vortz_l * dt_loc
    end do
    end do
    end do

    this%total_time = this%total_time + dt_loc
    this%dt = 0._rprec
    return
end if

! GPU build: every field sampled below is device-resident (!$acc declare
! create) and the host copies go stale after step 1. Refresh them before
! accumulating. Infrequent path (every tavg_nskip steps), full DtoH is fine.
!$acc wait(1)
!$acc update self(u, v, w, p, txx, txy, tyy, txz, tyz, tzz,                    &
!$acc             dudy, dudz, dvdx, dvdz, dwdx, dwdy, Cs_opt2)
#if defined(PPTURBINES) || defined(PPATM) || defined(PPLVLSET)
!$acc update self(fxa, fya, fza)
#endif
#endif
...
!$acc exit data delete(this%txx, this%tyy, this%tzz, this%txy, this%txz, this%tyz)
!$acc exit data delete(this%p, this%fx, this%fy, this%fz, this%cs_opt2)
!$acc exit data delete(this%vortx, this%vorty, this%vortz)
...
!$acc update self(this%u2, this%v2, this%w2, this%uv, this%uw, this%vw)
!$acc update self(this%txx, this%tyy, this%tzz, this%txy, this%txz, this%tyz)
!$acc update self(this%p, this%fx, this%fy, this%fz, this%cs_opt2)
!$acc update self(this%vortx, this%vorty, this%vortz)
#endif
