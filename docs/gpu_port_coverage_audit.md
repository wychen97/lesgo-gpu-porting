...
| CPS / concurrent precursor | `USE_CPS=ON` | CPS velocity exchange and fringe application have device-resident paths; scalar CPS is GPU-resident only with `PPSCALARS_GPU`. | Packaged velocity-only and scalar-coupled two-domain CPU/GPU examples passed field and kinetic-energy checks. | Retain production-size timing with and without scalar coupling. |
| HIT inflow | `USE_HIT=ON`, `inflow_type = 2` | HIT file/restart I/O remains host-owned; per-step plane interpolation and fringe blending are GPU-assisted in `hit_inflow_gpu.f90`. | The packaged `64^3` CPU/GPU example passed using the included exact `32^3` HIT input. | Retain a larger timing case before making a speedup claim. |
| Shifted inflow | shifted-inflow runtime path | Source contains GPU-aware branches for active LES GPU builds. | The packaged `64^3` CPU/GPU pair passed field and kinetic-energy checks. | Retain a larger timing case before making a speedup claim. |
| Sponge and Coriolis forcing | `use_sponge`, `coriolis_forcing` | Source contains GPU branches and reductions. | The packaged combined `64^3` CPU/GPU pair passed field and kinetic-energy checks. | Retain a larger timing case before making a speedup claim. |
...
| Coriolis | `coriolis_forcing`, `G`, `alpha`, PID keys | GPU branches exist and a fixed-geostrophic compact example is packaged; PID/interpolated modes still require separate targeted checks. |
| Flow condition | `inflow_type`, `lbc_mom`, `ubc_mom`, `use_sponge`, mean/random pressure force keys | Public cases now cover uniform, HIT, shifted, and CPS configurations plus common wall/sponge settings. Random forcing and less common wall settings still need targeted checks before speed claims. |
...
| Scalars | scalar boundary, lapse rate, initial profile, active/passive scalar, `Pr_sgs` | GPU source exists only when `USE_SCALARS_GPU=ON`; compact active/passive examples are packaged, while production-size validation remains a separate gate. |
| HIT | `inflow_type=2`, HIT dimensions/files | Optional HIT has a GPU helper path, prior validation, and a compact public input, but remains outside the standard four-case production gate. |
