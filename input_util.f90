...
!*******************************************************************************
! Navigation map:
!   - read_input_conf: top-level lesgo.conf parser
!   - block parsers: domain, model, coriolis, time, flow_cond, output,
!     level_set, turbines, and scalars
!   - validation: checkentry
!   - parser utilities: readline and parse_vector overloads
!
! Keep new configuration fields synchronized with param.f90, param_output.f90,
! docs/code_organization.md, and docs/environment_switches.md as appropriate.
...
do
    ...
    ! Each non-empty configuration line must begin with a recognized block tag.
...
! check if run-time number of processes agrees with nproc parameter
        ...
        ! Set the processor owned vertical grid spacing. Nz is the number of
        ! vertical cells in the input file; it must split exactly across ranks.
        ival_read = nz_tot
        if ( mod( ival_read, nproc ) /= 0 ) then
            call error( sub_name, 'Input Nz = ', ival_read,                   &
                ' must be divisible by nproc = ', nproc )
        endif
        nz = ival_read / nproc + 1
        nz_tot = ( nz - 1 ) * nproc + 1
