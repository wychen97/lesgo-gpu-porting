...
! Runtime `sgs_model` values read from lesgo.conf.
integer, parameter :: SGS_MODEL_SMAGORINSKY = 1
integer, parameter :: SGS_MODEL_STANDARD_DYNAMIC = 2
integer, parameter :: SGS_MODEL_SCALE_DEP_DYNAMIC = 3
integer, parameter :: SGS_MODEL_LAGRANGE_SIMILARITY = 4
integer, parameter :: SGS_MODEL_LAGRANGE_SCALE_DEP = 5
...
#ifdef PPSGS_GPU
! Experimental explicit-residency SGS route. These arrays are ordinary host
! allocatables with persistent OpenACC device mirrors. The existing managed
! memory declarations remain active when PPSGS_GPU is disabled.
!$acc declare create(S11, S12, S13, S22, S23, S33, Nu_t, Cs_opt2, S)
!$acc declare create(ee_now, u_bar, v_bar, w_bar, u_hat, v_hat, w_hat)
!$acc declare create(L11, L12, L13, L22, L23, L33)
!$acc declare create(M11, M12, M13, M22, M23, M33)
!$acc declare create(Q11, Q12, Q13, Q22, Q23, Q33)
!$acc declare create(N11, N12, N13, N22, N23, N33)
!$acc declare create(S11_bar, S12_bar, S13_bar, S22_bar, S23_bar, S33_bar)
!$acc declare create(S11_hat, S12_hat, S13_hat, S22_hat, S23_hat, S33_hat)
!$acc declare create(S_S11_bar, S_S12_bar, S_S13_bar, S_S22_bar, S_S23_bar, S_S33_bar)
!$acc declare create(S_S11_hat, S_S12_hat, S_S13_hat, S_S22_hat, S_S23_hat, S_S33_hat)
!$acc declare create(S_bar, S_hat)
!$acc declare create(F_LM, F_MM, F_QN, F_NN, Beta, Tn_all)
#ifdef PPDYN_TN
!$acc declare create(F_ee2, F_deedt2, ee_past)
#endif
#endif
...
! For dynamic models:
if (sgs_model .ne. SGS_MODEL_SMAGORINSKY) then
...
if ((sgs_model .eq. SGS_MODEL_LAGRANGE_SIMILARITY).or.                        &
    (sgs_model .eq. SGS_MODEL_LAGRANGE_SCALE_DEP)) then
...
if ((sgs_model .eq. SGS_MODEL_SCALE_DEP_DYNAMIC).or.                           &
    (sgs_model .eq. SGS_MODEL_LAGRANGE_SCALE_DEP)) then
...
#ifdef PPSGS_GPU
! Mirror initialized host values to persistent device storage. Subsequent SGS
! GPU routines update these arrays in place on the device.
!$acc update device(S11, S12, S13, S22, S23, S33, Nu_t, Cs_opt2, S)
if (sgs_model .ne. SGS_MODEL_SMAGORINSKY) then
    !$acc update device(ee_now, L11, L12, L13, L22, L23, L33)
    !$acc update device(M11, M12, M13, M22, M23, M33)
    !$acc update device(S11_bar, S12_bar, S13_bar, S22_bar, S23_bar, S33_bar)
    !$acc update device(S_S11_bar, S_S12_bar, S_S13_bar)
    !$acc update device(S_S22_bar, S_S23_bar, S_S33_bar)
    !$acc update device(S_bar, u_bar, v_bar, w_bar)
end if
...
if ((sgs_model .eq. SGS_MODEL_SCALE_DEP_DYNAMIC).or.                           &
    (sgs_model .eq. SGS_MODEL_LAGRANGE_SCALE_DEP)) then
    !$acc update device(S11_hat, S12_hat, S13_hat, S22_hat, S23_hat, S33_hat)
    !$acc update device(S_S11_hat, S_S12_hat, S_S13_hat)
    !$acc update device(S_S22_hat, S_S23_hat, S_S33_hat)
    !$acc update device(S_hat, u_hat, v_hat, w_hat)
end if
if ((sgs_model .eq. SGS_MODEL_SCALE_DEP_DYNAMIC).or.                           &
    (sgs_model .eq. SGS_MODEL_LAGRANGE_SCALE_DEP)) then
    !$acc update device(Q11, Q12, Q13, Q22, Q23, Q33)
    !$acc update device(N11, N12, N13, N22, N23, N33)
end if
#endif
