...
#endif
use lagrange_Sdep_gpu_m, only : lagrange_Sdep_gpu_init
...
! Initialize turbine forcing.  ATM and the legacy drag-disk turbine
! model are independent physical models; in an all-module compile gate,
! ATM owns the turbine path to avoid double initialization/forcing.
#if defined(PPTURBINES) && !defined(PPATM)
...
#ifdef PPSGS_GPU
! test_filter_init builds host kernels; mirror them to OpenACC device storage.
! test_filtermodule also does this internally, but keeping it here documents the
! initialization contract for the explicit-residency route.
...
#ifdef PPSGS_GPU
! Restart paths may initialize these arrays on host. Mirror the persistent SGS
...
#endif
if (sgs_model >= SGS_MODEL_LAGRANGE_SIMILARITY) call lagrange_Sdep_gpu_init()
#endif

! Initialize integral wall model when selected as the lower boundary model.
if (lbc_mom == 3) then
...
if (inflow_type == 4) call initialize_cps()
#endif
